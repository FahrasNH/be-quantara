...
test("runs end-to-end and returns stats/trades/equity", () => {
...
test("exits are SL/TP only — no 'Signal' reversal exit (live AF parity)", () => {
  const r = runRealBacktest({ entryCandles: entry, htfCandles: htf, strategyKey: "ADAPTIVE_FUSION", capital: 1000, config: { afMinVotes: 2, volSmaMultiplier: 1.0 } });
...
test("afMinVotes=3 (unanimity, live default) is at most as active as =2", () => {
  const r3 = runRealBacktest({ entryCandles: entry, htfCandles: htf, strategyKey: "ADAPTIVE_FUSION", capital: 1000, config: { afMinVotes: 3, volSmaMultiplier: 1.0 } });
  const r2 = runRealBacktest({ entryCandles: entry, htfCandles: htf, strategyKey: "ADAPTIVE_FUSION", capital: 1000, config: { afMinVotes: 2, volSmaMultiplier: 1.0 } });
  assert.ok(r3.stats.totalTrades <= r2.stats.totalTrades);
});

test("HTF directional block — no LONG while HTF BEARISH, no SHORT while HTF BULLISH", () => {
