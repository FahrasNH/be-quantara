...
(async () => {
  ...
  // ── 1. Reject the two losers → survivors are all winners ──────────────────
  {
    ...
    const res = await _applyGrokGate(trades, { strategyKey: "SMART_MONEY_CONCEPTS", grokConfirmFn: confirmFn });
  ...
  // ── 2. Fail-open: confirm fn throws → all trades kept ─────────────────────
  {
    ...
    const res = await _applyGrokGate(trades, { strategyKey: "SMART_MONEY_CONCEPTS", grokConfirmFn: boom });
  ...
  // ── 3. No decision for a signal → kept (per-signal fail-open) ─────────────
  {
    ...
    const res = await _applyGrokGate(trades, { strategyKey: "SMART_MONEY_CONCEPTS", grokConfirmFn: partial });
