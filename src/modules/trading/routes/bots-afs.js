...
module.exports = function createBotsRouter(helpers) {
  ...
  const { strategyGuard } = require("../../../shared/middleware/strategyGuard");
  const { strategyChangeLimiter, emergencyStopConfirmGuard } = require("../../../shared/middleware/strategyRateLimiter");
  const { analyzeStrategyFit } = require("../../../core/analytics-engine/strategyAnalysis");
  const { getMarketSnapshot, getPairTierMetrics } = require("../../research/services/MarketSnapshotService");
  const { issueConfirmToken, DEFAULT_MAX_AGE_MS } = require("../../../infrastructure/security/confirmToken");
  const { createBotOpLock } = require("../../../shared/middleware/botOpLock");
  ...
  router.get("/strategies/available", asyncHandler(async (req, res) => {
    ...
    const { listTiers, getTierConfig } = require("../../../core/risk-engine/tierConfig");
    const { isStrategyLiveReady } = require("../../users/services/entitlement");
