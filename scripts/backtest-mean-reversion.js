#!/usr/bin/env node
/**
 * Backtest MEAN_REVERSION Strategy (VAULT tier)
 *
 * Usage:
 *   node scripts/backtest-mean-reversion.js
 *   node scripts/backtest-mean-reversion.js --days 365 --capital 30000000
 *
 * Target: 55-60% win rate, 100-150% annual return, <10% max drawdown
 */
...
async function runBacktest() {
  console.log(`\n${"═".repeat(70)}`);
  console.log(`  MEAN_REVERSION Strategy Backtest (VAULT tier)`);
  console.log(`  Symbol: ${SYMBOL} | Days: ${DAYS} | Start Balance: $${START_BALANCE}`);
  console.log(`  Expected: Win Rate 55-60%, RR 1:3.0, Annual Return 100-150%`);
  ...
  for (let i = 50; i < candles.length; i++) {
    ...
    // Size: 1% risk per trade (VAULT tier ultra-conservative)
  ...
  if (trades.length === 0) {
    console.log("⚠️  No trades generated. Market may not be choppy enough.\n");
    return;
  }
  ...
  console.log(`  Win Rate       : ${winRate}%  (${wins.length}W / ${losses.length}L)  ${parseFloat(winRate) >= 55 ? "✅" : "⚠️"}`);
  console.log(`  Avg Win        : $${avgWin}`);
  console.log(`  Avg Loss       : $${avgLoss}`);
  console.log(`  Avg RR         : 1:${avgRR}  ${parseFloat(avgRR) >= 2.8 ? "✅" : "⚠️"}`);
  console.log(`  Profit Factor  : ${pf}  ${pf >= 1.8 ? "✅" : pf >= 1.0 ? "⚠️" : "❌"}`);
  console.log(`  Sharpe Ratio   : ${sr}  ${sr >= 1.2 ? "✅" : sr >= 0.8 ? "⚠️" : "❌"}`);
  console.log(`  Max Drawdown   : ${mdd}%  ${mdd <= 10 ? "✅" : mdd <= 15 ? "⚠️" : "❌"}`);
  console.log(`  Total PnL      : $${totalPnl}  (${roi}%)`);
  console.log(`  Final Balance  : $${balance.toFixed(2)}`);
  console.log(`─${"─".repeat(68)}`);

  // Acceptance check (VAULT tier criteria)
  const passWR  = parseFloat(winRate) >= 55;
  const passRR  = parseFloat(avgRR) >= 2.8;
  const passPF  = parseFloat(pf) >= 1.8;
  const passMDD = parseFloat(mdd) <= 10;
  const pass    = passWR && passRR && passPF && passMDD;

  console.log(`\nVAULT Tier Acceptance Criteria:`);
  console.log(`  ✅ Win Rate 55%+     : ${passWR ? "PASS" : "FAIL"} (${winRate}%)`);
  console.log(`  ✅ RR 1:2.8+         : ${passRR ? "PASS" : "FAIL"} (1:${avgRR})`);
  console.log(`  ✅ Profit Factor 1.8 : ${passPF ? "PASS" : "FAIL"} (${pf})`);
  console.log(`  ✅ Max Drawdown <10% : ${passMDD ? "PASS" : "FAIL"} (${mdd}%)`);
